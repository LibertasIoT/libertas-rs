...
/// This module provides functions for interacting with the standalone and indexed databases in the Libertas runtime. 
/// The standalone database allows you to store and retrieve individual pieces of data identified by a resource name and arguments, 
/// while the indexed database allows you to store and retrieve multiple records of data organized by an index value. This module includes functions for writing, reading, 
/// and removing data from both databases, as well as functions for opening handles to indexed data and querying for available data names. 
/// The data is encoded in Avro format when stored in the databases, so the functions in this module also handle encoding and decoding of data using the `AvroEncode` and `AvroDecode` traits. 
/// By using this module, you can easily manage your application's data within the Libertas runtime, allowing you to store state, keep track of historical changes, and query for specific pieces of data as needed./// 
/// 
...
/// The direction in which to read indexed data records from the indexed database, either above or below a specified index value. This enum is used as a parameter in the `libertas_data_read_indexed_range` function to determine whether to read records with index values greater than or equal to the specified starting index (`Above`) or to read records with index values less than or equal to the specified starting index (`Below`). The choice of direction allows you to control whether you want to read forward or backward from the starting index, which can be useful for different use cases when working with indexed data. For example, if you want to read the most recent records up to a certain point, you might use `Below` to read backward from a high index value. Conversely, if you want to read upcoming records starting from a certain point, you might use `Above` to read forward from a low index value.
#[repr(u8)]
pub enum IndexDirection {
    /// Read records with index values greater than or equal to the specified starting index. When this direction is specified in the `libertas_data_read_indexed_range` function, it will read records from the indexed database that have index values that are greater than or equal to the `index` parameter provided to that function. This means that if you specify a starting index value, the function will return records that have index values at that starting point and any index values above it. This can be useful when you want to read forward from a certain point in your indexed data, such as reading upcoming events or future data points based on their index values.
    Above,
...
/// A struct representing a piece of indexed data in the indexed database, containing the index value and the decoded data of type `T`. This struct is used to represent individual records of indexed data that are read from the indexed database using functions like `libertas_data_read_indexed_range`. The `index` field is a 64-bit integer that represents the index value associated with that piece of indexed data, which can be used to identify and query for specific records based on their index values. The `data` field contains the actual data associated with that index value, decoded into the specified type `T` that implements the `AvroDecode` trait. When you read indexed data from the database, it is stored in Avro format, so it needs to be decoded into a usable Rust type, and this struct serves as a convenient way to package both the index and the decoded data together when working with indexed records.
///     
...
/// A struct representing the name of a piece of data in the database, including the resource name and arguments. This struct is used to represent the names of data that are stored in the standalone and indexed databases, 
/// and it is returned when querying for available data names using functions like `libertas_data_get_names` and `libertas_data_get_indexed_names`. 
/// The `resource_name` field is a string that identifies the type of data, such as "player_position" or "game_score". The `arguments` field is a vector of `NotificationArgumentDecode` structs that further specify the data, 
/// which can include various types of data such as integers, strings, or booleans. The specific arguments needed to identify a piece of data will depend on how that data was originally written to the database. By using this struct, 
/// you can easily manage and query for available data in the database based on their resource names and arguments.
...
/// A struct containing statistics about a piece of indexed data in the indexed database, including the handle for the indexed data, the number of records, and the range of index values. This struct is returned when opening indexed data using `libertas_data_open_indexed`, and it provides important information about the indexed data that can be used for subsequent read or write operations. The `handle` field is a `LibertasDataStore` value that serves as a reference to the opened indexed data and is used in other functions to read or write records associated with that indexed data. The `count` field indicates how many records are currently stored in the indexed database for that piece of indexed data. The `min_index` and `max_index` fields indicate the range of index values for the records stored in the indexed database for that piece of indexed data, which can be useful for understanding the distribution of index values and for performing range queries on the indexed data. 
#[repr(C)]
pub struct IndexedDataStat {
    /// The handle for the opened indexed data, which is used for subsequent read or write operations on that indexed data. This handle is obtained when opening indexed data using `libertas_data_open_indexed` and is required for functions that read or write records associated with that indexed data, such as `libertas_data_read_indexed`, `libertas_data_read_indexed_range`, and `libertas_data_write_indexed`. The handle serves as a reference to the specific piece of indexed data in the indexed database that you have opened, allowing you to perform operations on it without needing to specify the resource name and arguments again.
    pub handle: LibertasDataStore,
    /// The number of records currently stored in the indexed database for the opened indexed data. This count indicates how many records are associated with the specific piece of indexed data that you have opened using `libertas_data_open_indexed`. The count can be useful for understanding how much data is stored for that indexed data and for making decisions about how to read or write records based on the number of existing records. For example, if the count is zero, it means there are no records currently stored for that indexed data, and you may want to write some records before attempting to read. Conversely, if the count is very high, you may want to read a range of records instead of trying to read them all at once.
    pub count: u64,
...
/// Note that indexed data names are not included in this list, use `libertas_data_get_indexed_names` to get those.
...
/// Note that standalone data names are not included in this list, use `libertas_data_get_names` to get those.
/// The returned data names are decoded from Avro format and returned as a vector of `DataName` structs.
/// 
/// # Returns
...
/// The database handle is obtained when opening indexed data using `libertas_data_open_indexed`, and the index range specifies which records to remove based on their index values. After this function is called, all indexed records in the specified database that have index values within the specified range will be removed from the indexed database.
/// 
/// # Arguments
/// * `db` - The handle of the indexed database from which to remove records. This handle is obtained by calling `libertas_data_open_indexed` with the appropriate resource name and arguments that identify the indexed data.
...
/// Writes a piece of indexed data to the indexed database, identified by the database handle and index value. The data is encoded in Avro format and sent to the device to perform the write operation. After this function is called, the specified indexed data will be available in the indexed database under the given index value and can be read using `libertas_data_read_indexed` with the same database handle and index value.
/// 
/// # Arguments
/// * `db` - The handle of the indexed database to which to write the record. This handle is obtained by calling `libertas_data_open_indexed` with the appropriate resource name and arguments that identify the indexed data.
...
/// 
/// # Arguments
/// * `db` - The handle of the indexed database from which to read records. This handle is obtained by calling `libertas_data_open_indexed` with the appropriate resource name and arguments that identify the indexed data.
...
/// 
/// # Arguments
/// * `db` - The handle of the indexed database from which to read the record. This handle is obtained by calling `libertas_data_open_indexed` with the appropriate resource name and arguments that identify the indexed data.
